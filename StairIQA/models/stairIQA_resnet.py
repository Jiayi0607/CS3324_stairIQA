...
class resnet50(torch.nn.Module):
    def __init__(self, pretrained = True):
        ...
        self.hyper1_1 = self.hyper_structure1(64,256)      #channels /4 *16 = *4, size no change
        self.hyper2_1 = self.hyper_structure2(256,512)     #channels /4 *8 = *2, size /2
        self.hyper3_1 = self.hyper_structure2(512,1024)    #channels /4 *8 = *2, size /2
    ...
    # bottleneck structure1(for hyper1_1): channels /4 *16 = *4, size no change
    ...
    # bottleneck structure2(for the other hyper): channels /4 *8 = *2, size /2
    ...
    def forward(self, x):
        ...
        x = self.feature_extraction_stem(x)    #ResNet part0

        x_hyper1 = self.hyper1_1(x)              # 64-in, 256-out
        x = self.feature_extraction1(x)        #ResNet part1


        x_hyper1 = self.hyper2_1(x_hyper1+x)     # 256-in, 512-out
        x_hyper2 = self.hyper2_2(x)              # 256-in, 512-out
        x = self.feature_extraction2(x)        #ResNet part2


        x_hyper1 = self.hyper3_1(x_hyper1+x)
        x_hyper2 = self.hyper3_2(x_hyper2+x)
        x_hyper3 = self.hyper3_3(x)
        x = self.feature_extraction3(x)        #ResNet part3
        ...
        x = self.feature_extraction4(x)        #ResNet part4

        # Before this, the update for x is the same as ResNet50. But the low-level, mid-level and high-level information are saved and synchronous in scale.
