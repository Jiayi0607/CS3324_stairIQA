...
if __name__ == '__main__':
    ...
    hyperParas = os.path.join(results_path, "hyperParas")
    with open(os.path.join(hyperParas, "hyperParameters.json"), 'w') as f:
        json.dump(vars(args), f, indent=4)

    best_all = np.zeros([10, 4])
    for exp_id in range(0,5):
        
        exp_results = {'batch_losses':[], 'epoch_losses':[], 'var_metrics':[]}
        ...
        for epoch in range(num_epochs):
            ...
            for i, (image, mos) in enumerate(train_loader):
                ...
                exp_results['batch_losses'].append(loss.item())
            ...
            exp_results['epoch_losses'].append(avg_loss)
            ...
            with torch.no_grad():
                ...
                exp_results['var_metrics'].append([test_SRCC, test_KRCC, test_PLCC, test_RMSE])
        ...
        trainingData = os.path.join(results_path, "trainingData")
        plt.figure(figsize=(10, 5))
        plt.plot(range(1, len(exp_results['batch_losses'])+1), exp_results['batch_losses'], label='Training Loss (per batch)')
        plt.xlabel('Step (batch index)')
        plt.ylabel('Loss')
        plt.title(f'Training Loss Curve (exp_id={exp_id})')
        plt.legend()
        plt.savefig(os.path.join(trainingData, f'loss_curve_exp_{exp_id}.png'))
        plt.close()

        np.savez(
            os.path.join(trainingData, f'exp_results_{exp_id}.npz'),
            batch_losses=np.array(exp_results['batch_losses']),
            epoch_losses=np.array(exp_results['epoch_losses']),
            var_metrics=np.array(exp_results['var_metrics'])
        )
    ...
    np.savez(
        os.path.join(trainingData, 'all_experiments_summary.npz'),
        best_all=best_all,
